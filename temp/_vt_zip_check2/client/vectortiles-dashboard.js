(function () {
  ...
  const pollJobUntilDone = async (jobId, { timeoutMs = JOB_WAIT_TIMEOUT_MS, intervalMs = 1500 } = {}) => {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
      const payload = await api(`${API_BASE}/jobs/${encodeURIComponent(jobId)}`);
    ...
    if (!actions.querySelector(`.${BTN_CLASS}`)) {
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (String(error?.message || '') === 'job_timeout') {
            showStatus(t('timeoutRunning'));
            return;
          }
          showStatus(`${t('failed')}: ${error?.message || error}`, true);
        } finally {
          btn.disabled = false;
          btn.textContent = old;
        }
  ...
  const mount = async () => {
    ...
    if (window.qtilerLang?.subscribe) {
      window.qtilerLang.subscribe(() => {
        registerInfoTab();
        document.querySelectorAll(`.${BTN_CLASS}`).forEach((btn) => { btn.textContent = t('generate'); });
