# Lantmäteriet Info Control for Qtiler2Hajk
...
1. In the Qtiler2Hajk Admin UI, open a map editor
2. Go to the "3. Controls & search" tab
3. In the "Custom Qtiler2Hajk Controls" section, enable **Lantmäteriet Search (Property & Address)**
...
Edit `plugins/Qtiler2Hajk/index.js` and replace the mock functions with real API calls. The control sends coordinates (lon, lat) and info type to the backend.
...
- **Endpoint**: `GET /plugins/Qtiler2Hajk/api/lantmateri-proxy`
...
This custom control is part of Qtiler2Hajk and is subject to the MPL 2.0 license.
