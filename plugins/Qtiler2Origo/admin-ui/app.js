...
legendLibraryGrid?.addEventListener('click', async (event) => {
  ...
  // Warn if the icon is referenced by layers in the map currently open in
  // the editor. Only the in-memory draft state can be checked reliably here.
  const item = (legendLibraryItems || []).find((it) => String(it?.id) === id);
  const itemUrl = String(item?.url || '').trim();
  const usingLayers = [];
  if (itemUrl) {
    for (const [key, rule] of Object.entries(publishState.mainRules || {})) {
      const ref = String(rule?.wmsLegendUrl || rule?.wmsLegendIcon || rule?.legendIcon || '').trim();
      if (ref && ref === itemUrl) {
        const title = String(rule?.title || key).trim();
        if (title) usingLayers.push(title);
      }
    }
  }
  const confirmMsg = usingLayers.length
    ? t('Qtiler2Origo.legend_delete_in_use', { n: usingLayers.length, layers: usingLayers.slice(0, 5).join(', ') + (usingLayers.length > 5 ? '…' : '') })
