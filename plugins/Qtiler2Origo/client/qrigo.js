(function () {
  ...
  const openServiceModal = ({ hasWfs, hasVectorTiles, tr, onConfirm }) => {
    ...
    const ensureModalStyles = () => {
      if (document.getElementById('Qtiler2Origo-service-modal-style')) return;
      const style = document.createElement('style');
      style.id = 'Qtiler2Origo-service-modal-style';
      style.textContent = `
        .Qtiler2Origo-modal{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:10050;padding:16px}
        .Qtiler2Origo-modal.modal.is-active{display:flex}
        .Qtiler2Origo-modal .modal-background{position:absolute;inset:0;background:rgba(10,10,10,.45)}
        .Qtiler2Origo-modal .modal-card{position:relative;max-width:460px;width:min(460px,calc(100vw - 32px));max-height:80vh;overflow:auto;margin:0;background:#ffffff;border:1px solid #d7dde7;border-radius:12px;box-shadow:0 16px 42px rgba(15,23,42,.3)}
        .Qtiler2Origo-modal .modal-card-head,.Qtiler2Origo-modal .modal-card-body,.Qtiler2Origo-modal .modal-card-foot{background:#ffffff;color:#1f2937}
        .Qtiler2Origo-modal .modal-card-head{border-bottom:1px solid #e5e7eb;padding:1rem 1.2rem}
        .Qtiler2Origo-modal .modal-card-foot{border-top:1px solid #e5e7eb;padding:1rem 1.2rem}
        .Qtiler2Origo-modal .modal-card-title{font-size:1.05rem;font-weight:700}
        .Qtiler2Origo-modal .modal-card-body{display:grid;gap:.85rem;padding:1.15rem 1.2rem}
        .Qtiler2Origo-service-grid{display:grid;grid-template-columns:1fr 1fr;gap:.5rem}
        .Qtiler2Origo-service-option{border:1px solid #dbe1ea;border-radius:8px;padding:.55rem .65rem;display:flex;gap:.45rem;align-items:center}
        .Qtiler2Origo-modal .Qtiler2Origo-btn{border:1px solid #cbd5e1;background:#fff;color:#1f2937;border-radius:8px;padding:.45rem .9rem;font-weight:600;cursor:pointer}
        .Qtiler2Origo-modal .Qtiler2Origo-btn:hover{background:#f8fafc}
        .Qtiler2Origo-modal .Qtiler2Origo-btn-primary{border-color:#0369a1;background:#0369a1;color:#fff}
        .Qtiler2Origo-modal .Qtiler2Origo-btn-primary:hover{background:#075985}
        @media (max-width:640px){.Qtiler2Origo-service-grid{grid-template-columns:1fr}}
      `;
    ...
    modal.className = 'modal is-active Qtiler2Origo-modal';
    modal.innerHTML = `
      ...
      <div class="modal-card">
        ...
        <section class="modal-card-body">
          ...
          <label class="Qtiler2Origo-service-option"><input type="checkbox" data-all="1"> <strong>All available services</strong></label>
          <div class="Qtiler2Origo-service-grid"></div>
        </section>
        <footer class="modal-card-foot" style="justify-content:flex-end">
          <button class="Qtiler2Origo-btn" data-cancel="1">${escapeHtml(tr ? tr('Cancel') : 'Cancel')}</button>
          <button class="Qtiler2Origo-btn Qtiler2Origo-btn-primary" data-open="1">${escapeHtml(tr ? tr('Open in Origo') : 'Open in Origo')}</button>
        </footer>
      </div>
    `;

    const grid = modal.querySelector('.Qtiler2Origo-service-grid');
    ...
    for (const svc of available) {
      ...
      label.className = 'Qtiler2Origo-service-option';
        ...
        () => {
          openServiceModal({
            ...
            onConfirm: (selectedServices) => {
              ...
              if (!selected.length) return;
              const url = '/Qtiler2Origo/preview2?project=' + encodeURIComponent(projectId)
