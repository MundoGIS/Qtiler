...
const enforceProjectAccess = (req, res, projectId, dataRoot, security) => {
  if (typeof security?.isEnabled !== 'function' || !security.isEnabled()) return true;
  const snapshot = readAccessSnapshot(dataRoot);
  if (userCanAccessProject(snapshot, req.user || null, projectId)) return true;
  res.status(req.user ? 403 : 401).json({ error: req.user ? 'forbidden' : 'auth_required' });
  return false;
};
...
.qtwc-catalog-footer a{color:#7dd3fc;text-decoration:underline}
  ...
  app.use('/Qtiler2qwc/3dtiles', (req, res, next) => {
    const projectId = decodeURIComponent(String(req.path || '').split('/').filter(Boolean)[0] || '');
    if (!projectId) return next();
    if (!enforceProjectAccess(req, res, projectId, dataRoot, security)) return;
    return next();
  }, express.static(tiles3dRoot, { index: false, fallthrough: true }));
  ...
  app.get('/Qtiler2qwc/search', async (req, res) => {
    try {
      ...
      if (!enforceProjectAccess(req, res, safeProject, dataRoot, security)) return;
