...
const SCHEMA = `
  ...
  CREATE TABLE IF NOT EXISTS projects (
    ...
    edit_users TEXT NOT NULL DEFAULT '[]',
...
export const getAuthDb = (dataDir) => {
  ...
  try {
    ...
    if (!userCols.includes('permissions')) {
      _db.exec("ALTER TABLE users ADD COLUMN permissions TEXT NOT NULL DEFAULT '[]'");
    }
    _db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_users_api_key_hash ON users(api_key_hash) WHERE api_key_hash IS NOT NULL");
    const projectCols = _db.prepare("PRAGMA table_info(projects)").all().map((c) => c.name);
    if (!projectCols.includes('edit_users')) {
      _db.exec("ALTER TABLE projects ADD COLUMN edit_users TEXT NOT NULL DEFAULT '[]'");
    }
    if (!projectCols.includes('edit_roles')) {
      _db.exec("ALTER TABLE projects ADD COLUMN edit_roles TEXT NOT NULL DEFAULT '[]'");
    }
  } catch (err) {
...
export const readProjectAccessFromDb = (dataDir) => {
  ...
  for (const r of rows) {
    projects[r.project_id] = {
      ...
      editUsers: JSON.parse(r.edit_users || '[]'),
      editRoles: JSON.parse(r.edit_roles || '[]')
    };
  }
  return { projects };
};
