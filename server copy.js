...
const findTileMatrixPresetById = (id) => {
  if (!id) {
    return null;
  }
  const presets = loadTileGridPresets();
  return presets.find((preset) => preset.id === id || preset.fileName === id) || null;
};
...
// Tile grid presets listing (must be before static middleware)
app.get(["/tile-grids", "/api/tile-grids"], (req, res) => {
  console.log(`[Qtiler] Request to ${req.path} from user ${req.user ? req.user.username : 'anonymous'}`);
  try {
    const filterCrs = typeof req.query.crs === "string" && req.query.crs.trim()
      ? req.query.crs.trim().toUpperCase()
      : null;
    const presets = loadTileGridPresets();
    const items = [];
    for (const meta of presets) {
      let raw = null;
      try {
        raw = fs.existsSync(meta.path) ? JSON.parse(fs.readFileSync(meta.path, "utf8")) : null;
      } catch (err) {
        raw = null;
      }
      const supported = (() => {
        if (raw && raw.supported_crs) {
          if (typeof raw.supported_crs === "string") return [raw.supported_crs];
          if (Array.isArray(raw.supported_crs)) return raw.supported_crs;
        }
        return meta.supportedCrs || [];
      })();
      ...
      items.push({
        ...
        title: (raw && (raw.title || raw.name)) || meta.id,
        supportedCrs: normalizedSupported,
        coordinateReferenceSystem: raw?.coordinateReferenceSystem || raw?.crs || null,
        projectId: raw?.project_id || raw?.projectId || null,
        autoGenerated: raw?.auto_generated === true
      });
    }
    // Siempre incluir todos los grids, aunque el proyecto no tenga capas cacheadas
    // Filtrar por CRS si se solicita
    const filtered = filterCrs
      ? items.filter((item) => Array.isArray(item.supportedCrs) && item.supportedCrs.includes(filterCrs))
      : items;
    return res.json({ grids: filtered, total: filtered.length, filterCrs: filterCrs || null });
  } catch (err) {
    return res.status(500).json({ error: "tile_grid_list_failed", details: err?.message || String(err) });
  }
});

app.use(express.static(publicDir, { index: false }));

// Endpoint para obtener tile grids disponibles filtrados por CRS
app.get("/api/tile-grids", (req, res) => {
  const requestedCrs = req.query.crs;
  if (!requestedCrs) {
    return res.status(400).json({ error: "CRS parameter required" });
  }
  ...
  for (const project of allProjects) {
    const cacheIndexPath = path.join(cacheDir, project.id, "index.json");
    if (!fs.existsSync(cacheIndexPath)) continue;

    try {
      const cacheIndex = JSON.parse(fs.readFileSync(cacheIndexPath, "utf8"));
      const projectCrs = cacheIndex.project?.crs;
      
      // Solo incluir proyectos con el mismo CRS
      if (projectCrs !== requestedCrs) continue;

      // Buscar tile_matrix_set en las capas cacheadas
      let tileMatrixSet = null;
      if (cacheIndex.layers && cacheIndex.layers.length > 0) {
        for (const layer of cacheIndex.layers) {
          if (layer.tile_matrix_set) {
      ...
      // Extraer información relevante del grid
      const matrices = tileMatrixSet.tileMatrix || tileMatrixSet.matrices || [];
      const resolutions = matrices.map(m => m.scaleDenominator ? m.scaleDenominator * 0.00028 : m.resolution).filter(Boolean);
      const minZoom = matrices.length > 0 ? Math.min(...matrices.map(m => parseInt(m.identifier || m.id || 0))) : 0;
      const maxZoom = matrices.length > 0 ? Math.max(...matrices.map(m => parseInt(m.identifier || m.id || 0))) : 0;

      // Usar projectId como parte del id único para evitar colisiones
      const gridId = `${project.id}:${tileMatrixSet.identifier || 'grid'}`;
      ...
      availableGrids.push({
        ...
        origin: tileMatrixSet.origin,
        resolutions: resolutions,
        minZoom: minZoom,
        maxZoom: maxZoom,
        tileMatrixCount: matrices.length
      });
    } catch (err) {
      console.warn(`[tile-grids] Failed to read cache index for project ${project.id}:`, err.message);
    }
  }

  // Ordenar por nombre de proyecto para presentación consistente
  availableGrids.sort((a, b) => (a.projectName || a.projectId).localeCompare(b.projectName || b.projectId));

  console.log(`[tile-grids] Found ${availableGrids.length} grids for CRS ${requestedCrs}:`, availableGrids.map(g => g.projectId));
  
  res.json({ grids: availableGrids, crs: requestedCrs });
});
...
const defaultProjectConfig = (projectId) => ({
  ...
  cachePreferences: {
    mode: "xyz",
    tileCrs: "EPSG:3857",
    tileMatrixPreset: null,
    tileGridSourceProject: null,
    allowRemote: false,
    throttleMs: 0,
    updatedAt: null
  },
...
const presetSupportsCrs = (presetId, crs) => {
  if (!presetId || !crs) return false;
  const preset = getTileMatrixPresetRaw(presetId);
  if (!preset) return false;
  const normalizedCrs = typeof crs === "string" ? crs.trim().toUpperCase() : "";
  if (!normalizedCrs) return false;
  const supportedList = Array.isArray(preset.supported_crs)
    ? preset.supported_crs
    : (typeof preset.supported_crs === "string" ? [preset.supported_crs] : (preset.coordinateReferenceSystem ? [preset.coordinateReferenceSystem] : []));
  return supportedList.some((item) => typeof item === "string" && item.trim().toUpperCase() === normalizedCrs);
};
...
const recordOnDemandRequest = (projectId, targetMode, targetName) => {
  ...
  if (!tileMatrixPresetId && configSnapshot?.cachePreferences) {
    tileMatrixPresetId = normalizePresetName(
      configSnapshot.cachePreferences.tileMatrixPreset
      || configSnapshot.cachePreferences.tile_matrix_preset
    );
  }
...
const buildProjectConfigPatch = (input = {}) => {
  ...
  if (input.cachePreferences && typeof input.cachePreferences === "object") {
    ...
    if (typeof prefs.tileMatrixPreset === "string") patch.cachePreferences.tileMatrixPreset = prefs.tileMatrixPreset;
    if (typeof prefs.tile_matrix_preset === "string") patch.cachePreferences.tileMatrixPreset = prefs.tile_matrix_preset;
    if (typeof prefs.tileGridSourceProject === "string") patch.cachePreferences.tileGridSourceProject = prefs.tileGridSourceProject;
    if (typeof prefs.tile_grid_source_project === "string") patch.cachePreferences.tileGridSourceProject = prefs.tile_grid_source_project;
...
const pickBootstrapTileProfile = (candidateList = [], autoGenExtent = null, projectId = null) => {
  ...
  for (const candidate of candidates) {
    ...
    // First, check if this project already has its own auto-generated grid
    let preset = null;
    if (projectId) {
      const safeCrs = normalized.replace(/[^a-zA-Z0-9]/g, "_");
      const safeProject = projectId.replace(/[^a-zA-Z0-9]/g, "_");
      const projectSpecificId = `${safeCrs}_${safeProject}`;
      preset = findTileMatrixPresetById(projectSpecificId);
    }
    
    // If not found, look for any preset matching this CRS (for manually created grids)
    if (!preset) {
      preset = findTileMatrixPresetForCrs(normalized);
      // If we found a preset but it belongs to another project (auto-generated), ignore it
      if (preset && preset.fileName) {
        const presetPayload = loadBootstrapPresetDefinition(preset);
        if (presetPayload?.auto_generated && presetPayload?.project_id && presetPayload.project_id !== projectId) {
          preset = null; // Ignore other project's auto-generated grid
        }
      }
    }
    ...
    if (!preset && normalized !== "EPSG:3857" && autoGenExtent) {
      ...
      // Re-search for the newly created project-specific grid
      if (projectId) {
        const safeCrs = normalized.replace(/[^a-zA-Z0-9]/g, "_");
        const safeProject = projectId.replace(/[^a-zA-Z0-9]/g, "_");
        const projectSpecificId = `${safeCrs}_${safeProject}`;
...
app.post("/projects", requireAdmin, (req, res) => {
  projectUpload.single("project")(req, res, async (err) => {
    ...
    // If replacing, delete existing project completely
    if (shouldReplace && fs.existsSync(targetPath)) {
      try {
        // Delete project file
        await fs.promises.unlink(targetPath);
        
        // Delete data directory (cache, config, etc.)
        const dataPath = path.join(dataDir, projectId);
        if (fs.existsSync(dataPath)) {
          await fs.promises.rm(dataPath, { recursive: true, force: true });
        }
        
        // Delete auto-generated tile-grids for this project
        if (fs.existsSync(tileGridDir)) {
          const gridFiles = fs.readdirSync(tileGridDir);
          for (const gridFile of gridFiles) {
            if (!gridFile.toLowerCase().endsWith('.json')) continue;
            const gridPath = path.join(tileGridDir, gridFile);
            try {
              const gridContent = JSON.parse(fs.readFileSync(gridPath, 'utf8'));
              if (gridContent.auto_generated && gridContent.project_id === projectId) {
                await fs.promises.unlink(gridPath);
                console.log(`[replace] Deleted auto-generated grid: ${gridFile}`);
              }
            } catch (gridErr) {
              console.warn(`[replace] Failed to check/delete grid ${gridFile}:`, gridErr);
            }
          }
        }
        
        // Clean up references in other projects
        const allProjects = listProjects();
        for (const otherProject of allProjects) {
          if (otherProject.id === projectId) continue;
          const configPath = path.join(dataDir, otherProject.id, 'config.json');
          if (fs.existsSync(configPath)) {
            try {
              const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
              if (config.cachePreferences?.tileGridSourceProject === projectId) {
                delete config.cachePreferences.tileMatrixPreset;
                delete config.cachePreferences.tileGridSourceProject;
                fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
                console.log(`[replace] Cleaned grid reference in project ${otherProject.id}`);
              }
            } catch (configErr) {
              console.warn(`[replace] Failed to clean references in ${otherProject.id}:`, configErr);
            }
          }
        }
        
        console.log(`[replace] Successfully deleted existing project: ${projectId}`);
        invalidateTileGridCaches();
      } catch (deleteErr) {
        console.error(`[replace] Failed to delete existing project ${projectId}:`, deleteErr);
        return res.status(500).json({ error: "delete_failed", details: String(deleteErr) });
      }
    } else if (!shouldReplace) {
      // Original behavior: add suffix if exists
      let suffix = 1;
...
app.post("/projects/check-exists", requireAdmin, (req, res) => {
  ...
  const projectId = sanitizeProjectId(path.basename(filename, ext));
...
// Endpoint para verificar dependencias de grid antes de eliminar
app.get("/projects/:id/grid-dependencies", requireAdmin, (req, res) => {
    ...
    for (const otherProject of allProjects) {
      ...
      try {
        ...
        if (config.cachePreferences?.tileGridSourceProject === projectId) {
          dependencies.push({
            id: otherProject.id,
            name: otherProject.name || otherProject.id
          });
        }
      } catch (configErr) {
        console.warn(`[dependencies] Failed to check config for project ${otherProject.id}:`, configErr);
      }
    }
  } catch (err) {
    console.error('[dependencies] Failed to check grid dependencies:', err);
    return res.status(500).json({ error: "dependency_check_failed", details: String(err) });
  }
  
  res.json({ projectId, dependencies, hasDependencies: dependencies.length > 0 });
});
...
app.delete("/projects/:id", requireAdmin, (req, res) => {
  ...
  // Limpiar referencias al grid de este proyecto en otros proyectos
  try {
    const allProjects = listProjects();
    for (const otherProject of allProjects) {
      if (otherProject.id === proj.id) continue;
      ...
      try {
        const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        if (config.cachePreferences?.tileGridSourceProject === proj.id) {
          // Resetear el grid a auto
          delete config.cachePreferences.tileMatrixPreset;
          delete config.cachePreferences.tileGridSourceProject;
          fs.writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf8');
          console.log(`[cleanup] Reset tile grid reference in project ${otherProject.id} (was referencing deleted project ${proj.id})`);
        }
      } catch (configErr) {
        console.warn(`[cleanup] Failed to check/update config for project ${otherProject.id}:`, configErr);
      }
    }
  } catch (err) {
    console.warn('[cleanup] Failed to clean grid references in other projects:', err);
  }
...
app.post("/generate-cache", requireAdmin, (req, res) => {
  ...
  if (projectId) {
    ...
    projectKey = proj.id;
    try {
      projectConfig = readProjectConfig(projectKey);
    } catch (err) {
      projectConfig = null;
    }
  }
  ...
  const cachePrefPreset = normalizePresetName(
    projectConfig?.cachePreferences?.tileMatrixPreset
    || projectConfig?.cachePreferences?.tile_matrix_preset
  );
  ...
  const tileCrsCandidates = [
    ...
    req.body?.cache_crs,
    projectConfig?.cachePreferences?.tileCrs,
  ...
  const selectCompatiblePreset = (presetId) => {
    if (!presetId) return null;
    if (matchCrs && !presetSupportsCrs(presetId, matchCrs)) {
      console.warn(`[cache] Ignoring preset ${presetId} because it does not declare CRS ${matchCrs}`);
      return null;
    }
    return presetId;
  };

  let effectiveTileMatrixPreset = selectCompatiblePreset(requestedTileMatrixPreset)
    || selectCompatiblePreset(existingPreset)
    || selectCompatiblePreset(cachePrefPreset)
    || "";

  if (!effectiveTileMatrixPreset && matchCrs) {
    const presetMatch = findTileMatrixPresetForCrs(matchCrs);
    if (presetMatch) {
  ...
  if (projectKey) {
    try {
      ...
      const patch = {
        ...
        cachePreferences: {
          ...(projectConfig?.cachePreferences || readProjectConfig(projectKey)?.cachePreferences || {}),
